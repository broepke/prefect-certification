import httpx
from prefect import flow, task
from prefect.tasks import exponential_backoff


@task
def fetch_weather(lat: float, lon: float):
    base_url = "https://api.open-meteo.com/v1/forecast/"
    temps = httpx.get(
        base_url,
        params=dict(latitude=lat, longitude=lon, hourly="temperature_2m"),
    )
    forecasted_temp = float(temps.json()["hourly"]["temperature_2m"][0])
    print(f"Forecasted temp C: {forecasted_temp} degrees")
    return forecasted_temp


@task
def save_weather(temp: float):
    with open("102_weather.csv", "w+") as w:
        w.write(str(temp))
    return "Successfully wrote temp"


@flow(retries=3, log_prints=True, retry_delay_seconds=exponential_backoff(backoff_factor=2))
def pipeline(lat: float = 38.9, lon: float = -77.0):
    raise Exception("This is a test exception")
    temp = fetch_weather(lat, lon)
    result = save_weather(temp)
    print(result)
    return result


if __name__ == "__main__":
    pipeline.serve(name="102 Weather Failure")
